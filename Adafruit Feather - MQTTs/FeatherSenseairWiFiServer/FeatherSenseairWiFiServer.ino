...
int intervalCfg = 360000;  // Read timing configuration by FTP every 6 minutes
int timerLastCfg = 0;
...
int currentLine = 0;
String currentFile;
// int MaxLineNum = 14400;                // maximum line number of one file, almost one day
int MaxLineNum = 10; 
int IntervalDelete = 7;              // 7 day
int CountLastDelete = 0;
int init_line = 1;
int end_line = 10;
// bool newfile = false;
 ...
 */
 ...
 Serial.println(CountLastDelete);
    // Detect the expired files and delete it
    if (CountLastDelete >= 14) {
      Serial.println("Start to delete expired files");
      String FileContent;
      CountLastDelete = 0;
      modem.readClock(1, returnString);
      int length = returnString.length();
      String year_str = returnString.substring(0,2);
      String month_str = returnString.substring(4,2);
      String day_str = returnString.substring(length-2);
      ...
      Serial.print("day");
      Serial.print(day);
      int CurrentNum = day + month*30 + year*365;
      Serial.print("Current time is:");
      Serial.print(CurrentNum);
      File myFile = SD.open("DTS.TXT");
      while (myFile.available()) {
        String line = myFile.readStringUntil('\n');
        Serial.println(line);
        int length = line.length();
        String date_line = line.substring(length - 7);
        Serial.print(date_line);
        int lengthd = date_line.length();
        String yearstr = date_line.substring(0,2);
        String monthstr = date_line.substring(4,2);
        String daystr = date_line.substring(lengthd - 3);
        ...
        Serial.print("day");
        Serial.print(day);
        int FileNum = day + month*30 + year*365;
        Serial.print("File time is:");
        Serial.print(FileNum);
        if (CurrentNum-FileNum>=7){
          Serial.print("Time Difference is:");
          Serial.print(CurrentNum-FileNum);
           int dashIndex = line.indexOf('-');
           if (dashIndex != -1) {
           String curfile = line.substring(0, dashIndex);
           logger.fileRemove_Specified(0, curfile);
           }
        }
        else{
          FileContent = FileContent + line + '\n';
          Serial.print("keep contents are:");
          Serial.print(FileContent);
        }
      }
      myFile.close();
      logger.fileRemove_Specified(0, "DTS.TXT");
      File newFile = SD.open("DTS.TXT", FILE_WRITE);
      if (newFile) {
        newFile.println(FileContent);
        Serial.print("successfully write");
        newFile.close();
      }
    }
    ...
    if ((millis()-timerLastRead) > intervalData){
      ...
      Serial.print("Progress in single-cycle data collection = ");
  ...
  //     returnValue = modem.mqttWrite(dataString, 1);
    ...
    if (numCollect==10){
      // Serial.print("!!!!!!Data transfer process starts!!!!!!");
      numCollect = 0;
      Serial.println(modem.enableIP());
        statusModem = modem.init();
        int restart_time = 0;
    ...
    // 1. check mqtt status
        ...
        if (end_line >= MaxLineNum){
          modem.readClock(1, returnString);
          String filestamp = logger.fileNameString() + "-";
          filestamp.concat(returnString);
          logger.DTSADD(filestamp, 0);
          logger.fileNewName();
          Serial.println("new file csv");
          CountLastDelete = CountLastDelete + 1;
        }
        // currentFile;
        // modem.powerToggle();
        //  delete now file
        modem.mqttDisconnect();
        Serial.println(modem.disableIP()); //has some error
        delay(5000);
    }
}
